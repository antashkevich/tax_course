...
export const Filter: FC<FilterPopsType> = ({
  ...
  return (
    <div className={styles.filterContainer}>
      {(Object.keys(ProductsCategories) as ProductCategoriesKeys[]).map(
        (category) => (
          <button
            className={cn(styles.buttonFilter, {
              [styles.buttonFilterActive as string]:
                category === buttonActiveClass,
            })}
            onClick={() => filterCategoryProducts(category)}
            key={category}
          >
